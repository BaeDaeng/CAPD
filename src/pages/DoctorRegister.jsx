...
export default function DoctorRegister() {
  ...
  const handleSubmit = (e) => {
    ...
    alert("의사 회원가입이 완료되었습니다. 로그인해주세요.");
    navigate('/login');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-lg p-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">의사 회원가입</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="의사번호" name="doctorNumber" placeholder="면허번호 등을 입력" onChange={handleChange} required />
          <Input label="이름" name="name" placeholder="성함을 입력하세요" onChange={handleChange} required />
          <Input label="이메일" name="email" type="email" placeholder="doctor@hospital.com" onChange={handleChange} required />
